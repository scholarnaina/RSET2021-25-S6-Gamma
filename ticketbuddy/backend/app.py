...
def create_sample_events():
    ...
    sample_events = [
        Event(
            ...
            name='Sonu Nigam',
            genre='Soul',
            date=datetime.now()+ timedelta(days=2),  # Assuming you want the date part of the datetime
            datetime=datetime.now()+ timedelta(days=1, minutes=1),
            r_price=2000.0,
            p_price=3000.0,
        ...
        Event(
            ...
            r_price=2000.0,
            p_price=3000.0,
            image_link="https://media.bleacherreport.com/w_800,h_533,c_fill/br-img-slides/003/923/481/c1cc332af8a10fcee3a712341ee80cc6_crop_exact.jpg"
        ),
        Event(
            v_id=3,
            ...
            r_price=2500.0,
            p_price=3500.0,
            image_link="https://media.bleacherreport.com/w_800,h_533,c_fill/br-img-slides/003/923/477/145b68831a734768dd628091dca8e204_crop_exact.jpg"
        ),
        Event(
            v_id=2,
            name='Troye Sivan',
        ...
        Event(
            ...
            r_price=1500.0,
            p_price=1800.0,
            image_link="https://res.cloudinary.com/dn29xlaeh/image/upload/q_75,h_1400,fl_lossy/beatgig-updated/Rj19bPlBB5xgRbcKCIIl7TFtK4xSrr"
        ),
        Event(
            v_id=2,
            name='Borussia Dortmund vs FC Bayern',
            genre='Sports',
            date=datetime.now()+ timedelta(days=2),  # Assuming you want the date part of the datetime
            datetime=datetime.now()+ timedelta(days=1, minutes=1),
            r_price=2000.0,
            p_price=3000.0,
            image_link="https://media.bleacherreport.com/w_800,h_533,c_fill/br-img-slides/003/923/483/2e58725712e9fbd8c879e02294832b26_crop_exact.jpg"
        )
    ]
...
@app.route('/cevent1', methods=['GET'])
def create_sample_events1():
    # Insert sample values into the Event table
    sample_events1 = [
        Event(
            v_id=3,
            name='Real Madrid vs Liverpool FC',
            genre='Sports',
            date=datetime.now(),  # Assuming you want the date part of the datetime
            datetime=datetime.now()+ timedelta( seconds=20),  # Assign the datetime value
            r_price=2000.0,
            p_price=3000.0,
            image_link="https://media.bleacherreport.com/w_800,h_533,c_fill/br-img-slides/003/923/478/84ef8c7fe62384d795d0308b466e0139_crop_exact.jpg"
        ),
        Event(
            v_id=3,
            name='FC Bayern vs RB Leipzig',
            genre='Sports',
            date=datetime.now()+ timedelta(days=2),  # Assuming you want the date part of the datetime
            datetime=datetime.now()+ timedelta(days=1, minutes=1),
            r_price=1500.0,
            p_price=1800.0,
            image_link="https://media.bleacherreport.com/w_800,h_533,c_fill/br-img-slides/003/923/482/b712789f32aed6421955315a7953bb04_crop_exact.jpg"
        ),
        Event(
            v_id=3,
            name='070 Shake',
    ...
    db.session.add_all(sample_events1)
    db.session.commit()
...
def no_auction():
    ...
    e_id = data.get('e_id')
    u_id = data.get('u_id')
    bid_count = Bid.query.filter_by(u_id=u_id, e_id=e_id).count()
    total_tickets = len(t_id)
    
    if bid_count + total_tickets <= 10:
        for i in t_id:
            ...
            if ticket:
                ...
                return jsonify({"error": "Ticket not found with ID {}".format(i)}), 404
        db.session.commit()
        return jsonify({"message": "Tickets selected successfully"})
    else:
        total_tickets=0
        return jsonify({"error": "Exceeded maximum allowed bids of 10"}), 400
...
def check_auction_end():
    ...
    for i in event:
        if datetime.now()>=i.datetime and i.status=="notcompleted":
            i.status="completed"
